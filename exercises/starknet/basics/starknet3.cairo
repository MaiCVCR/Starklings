...
mod ProgressTracker {
    ...
    struct Storage {
        ...
        progress: LegacyMap::<ContractAddress, felt252>,
    }
    ...
    impl ProgressTrackerImpl of super::IProgressTracker<ContractState> {
        fn set_progress(ref self: ContractState, user: ContractAddress, new_progress: u16) { // TODO: assert owner is calling
        
            let caller = get_caller_address();
            assert(caller == self.contract_owner.read(), 'Is not the owner');
        
            // TODO: set new_progress for user,
            self.progress.write(user, new_progress.into());
        }

        fn get_progress(self: @ContractState, user: ContractAddress) -> u16 { // Get user progress
            self.progress.read(user).try_into().unwrap()
        }
